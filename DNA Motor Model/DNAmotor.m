...
clear;clc;
n = 20; % size of DNA array
N = 1e4; % size of RNA array (surface)
ne = 1e2; % number of enzymes
brownianFrequency = 20; % s^-1, particle vibration
Kon = 1; % s^-1
Koff = 3; % s^-1
Kcat = 3;  % s^-1
timeStep = .01; % s, resolution
tMax = 1000; % s, total time simulated
density = 1;
nSpan = n;
...
while currentTime < tMax
    %current_time
    fprintf('\b\b\b\b%3d%%',floor(currentTime/tMax*100))
        ...
        for j = 1:n
            ...
            elseif (DNA(i,j)==1 && subRNA(i,j)==1) % if both  bound
                ...
                if (t_to_unbind < t_to_finish_cutting) % if time to unbind is faster
                    ...
                    for ie = 1:ne
                        if (enzymes(ie)==0 && rand<bindingProbabilities(i,j)/sum(sum(bindingProbabilities)))
            ...
            elseif (subRNA(i,j)==2) % if RNA gone
                % then do nothing
            end
    ...
    % update plot if it moved
    if (~isequal(lastPosition, currentPosition))
        title(sprintf('Simulation Progress: \n %3d%%  \n  Simulation Time: %3d%',floor(currentTime/tMax*100)), currentTime);
        addpoints(al, p(end,1), p(end,2)); hold on; drawnow;
    end
